...
const TeacherStudents = () => {
  ...
  const filteredStudents = students.filter(s => {
    ...
    if (filterGender && s.user.gender.toLowerCase() !== filterGender.toLowerCase()) return false;
    if (
      filterClass &&
      (
        (typeof s.class === 'object' && s.class && '_id' in s.class && s.class._id !== filterClass) ||
        (typeof s.class === 'string' && s.class !== filterClass)
      )
    ) return false;
    return true;
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editStudent) {
        ...
        toast({ title: 'Student updated', description: `${form.name} has been updated successfully.`, status: 'success' });
      } else {
        ...
        const studentData = await getAllStudents();
        setStudents(Array.isArray(studentData) ? studentData : studentData.users);
        toast({ title: 'Student created', description: `${form.name} has been added successfully.`, status: 'success' });
      }
      setShowDialog(false);
      setEditStudent(null);
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const studentData = await getAllStudents();
      setStudents(Array.isArray(studentData) ? studentData : studentData.users);
      toast({ title: 'Student deleted', description: 'Student has been deleted successfully.', status: 'success' });
    } catch {
  ...
  return (
    <Layout role="teacher" title="Students" subtitle="View and manage students">
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <select value={form.gender} onChange={e => setForm(f => ({ ...f, gender: e.target.value }))} className="w-full rounded-lg border p-2" required>
              <option value="male">Male</option>
              <option value="female">Female</option>
              <option value="other">Other</option>
            </select>
